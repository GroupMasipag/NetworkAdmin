...
init_db.py file - dito database - currently you can use sqlLite nalang muna lloydie para hindi malakas kain sa storage ng system ( switch nalang pag ok na )

GUIDE -- 
We are using SQLite for this project so we do not have to configure heavy external database servers. The database will live locally in a file called `network_defense.db`.

Flask-CORS - Lock Down API Access

**1. Initialize the Database**
Before you can write any SQL in the app, you need to create the `.db` file. 
...
Inside `app.py`, you will need to import `sqlite3` and create a connection helper function.

**3. Update the Routes**
Swap out the simulated data in our API routes with real SQL queries:
